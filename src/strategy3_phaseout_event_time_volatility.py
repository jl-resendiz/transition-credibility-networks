"""Event-time exposure regression with monthly realized volatility.

Compute monthly realized volatility from daily AR (vwretd-adjusted),
then estimate vol_{j,tau} = a_tau + b_tau * exposure_{j,e} + eps
with event-clustered SEs.

Outputs: results/summaries/strategy3_phaseout_event_time_vol_{transform}.csv
"""
import csv
import math
import os
...
vwretd = load_ff_factors_daily(raw_path('factors', 'F-F_Research_Data_Factors_daily.csv'))
if not vwretd:
    raise RuntimeError('Missing F-F daily factors for vwretd.')

# Build monthly volatility per gvkey
monthly_vol = defaultdict(dict)
for gk, dct in daily_ret.items():
    by_month = defaultdict(list)
    for d, r in dct.items():
        if d not in vwretd:
            continue
        ym = d[:7]
        ar = r - vwretd[d]
        by_month[ym].append(ar)
    for ym, vals in by_month.items():
        if len(vals) < 5:
            continue
        mean = sum(vals) / len(vals)
        var = sum((v - mean) ** 2 for v in vals) / max(len(vals) - 1, 1)
        monthly_vol[gk][ym] = math.sqrt(var)
    ...
    for fm_gk in event_gvkeys:
        ...
        for gk in candidate_firms:
            ...
            for tau in range(TAU_START, TAU_END + 1):
                ...
                if gk not in monthly_vol or ym not in monthly_vol[gk]:
                    continue
                vol = monthly_vol[gk][ym]
                rows_by_tau[tau].append((vol, w))
...
out_path = results_path('summaries', f'strategy3_phaseout_event_time_vol{suffix}{tf}{bo}.csv')
