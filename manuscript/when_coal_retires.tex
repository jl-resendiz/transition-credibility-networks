...
A potential identification threat is that the retirement decision is not randomly assigned. Firms may retire plants in response to declining profitability, regulatory pressure, or shifting demand. If markets anticipate retirements, prices adjust before the event window opens, attenuating measured CAR toward zero. This anticipation bias works against finding a significant fuel channel: any significant coefficient understates the full repricing. Even when retirements are anticipated, gradual information diffusion \citep{HongStein1999, CohenFrazzini2008} produces delayed cross-sectional effects: the daily event-time path in Section~\ref{sec:daily_path} bounds the pre-event drift at $-3.07$ cumulative percentage points, leaving the announcement-window effect of $-0.61$ as residual surprise.
...
This section reports the paper's central empirical findings. The central empirical fact, established in Section~\ref{sec:fuel_results} and reinforced throughout, is that fuel-mix similarity carries a large negative coefficient on cross-sectional CARs while geographic proximity is statistically indistinguishable from zero across the three inference methods reported in Table~\ref{tab:channel_decomp}. Section~\ref{sec:fuel_results} presents the headline channel decomposition; Sections~\ref{sec:geo_role}--\ref{sec:esg} supply the supporting geographic and ESG analyses; Sections~\ref{sec:learning}--\ref{sec:inst_heterogeneity} develop the geographic and institutional heterogeneity that identifies the propagation mechanism; Sections~\ref{sec:multifactor}--\ref{sec:anomaly_vs_risk} extend the headline to multi-factor returns, daily-resolution event time, and the anomaly-versus-risk demarcation; Section~\ref{sec:robustness} reports the robustness battery.

Table~\ref{tab:summary_stats} reports descriptive statistics for the key variables.
...
The difference is positive and significant under event-clustered inference ($t = 7.56$, $p < 0.001$): fuel-similar peers experience more negative returns than geographic neighbours.
...
\paragraph{Synthesis.}
Of the four country-level candidates, saturation is the only one that survives both diagnostics: regulation is rejected by the within-US restructured-versus-regulated split, and informational efficiency and country-level ownership concentration are weakened by the strong developed-ex-US effect. Saturation as a within-jurisdiction phenomenon is directionally supported but not separately identified; saturation as an across-jurisdiction phenomenon (US versus non-US, where US has accumulated many more retirements) is supported by the data. The within-firm institutional-ownership analysis in Section~\ref{sec:inst_heterogeneity} refines this picture: at the firm level the US null also reflects masked heterogeneity across ownership concentrations, with the two effects operating jointly.
...
To complement the four-month CAR with daily-resolution evidence, I trace the cross-sectional Fama-MacBeth coefficient on $w^{\text{fuel}}_i$ at each daily offset $\tau \in [-21, +21]$ around the announcement date. All 179 first-mover events have YYYY-MM-DD announcement dates from EIA-860 filings, IRP disclosures, or PSC settlements; daily firm-level returns and Fama-French three-factor adjustment permit an event-time path test.
...
Two readings are consistent with the $H$-aggregate decay pattern: (i) discrete information arrival followed by persistent repricing, where the $H \geq 12$ standard errors swamp any genuine continuation; (ii) partial mispricing that decays as arbitrageurs correct the under-reaction. Crossing the horizon decomposition with the 13F institutional-ownership terciles (Appendix~\ref{app:horizon_ownership}) sharpens the demarcation. At the headline window ($H = 3$), $\hat\gamma^{T3}_{\text{fuel}}(3) = -7.86$ and $\hat\gamma^{T1}_{\text{fuel}}(3) = +2.04$, with a difference of $-9.90$ (Welch $t = -4.12$). At longer horizons, both terciles' point estimates are statistically indistinguishable from zero with wide standard errors (T3 NW SE rises from 2.29 at $H = 3$ to 13.41 at $H = 24$). The pattern supports a layered reading: the T1 sign-reversal at the headline window decays toward zero by $H = 24$ (consistent with retail-flow misallocation correction), while T3 attains its peak at $H = 3$ and reverts thereafter (consistent with discrete information arrival followed by rapid pricing in concentrated-ownership firms). Both effects operate; the decomposition rejects a pure-systematic-risk reading in which T1 and T3 should track each other across horizons.
...
\section{Horizon-by-Tercile Decomposition}\label{app:horizon_ownership}

For each post-event horizon $H \in \{1, 3, 6, 12, 24\}$ months and each HHI tercile of US-listed firms with 13F coverage, I run cross-sectional Fama-MacBeth regressions of $\mathrm{CAR}_{ie}^{[-1, +H]}$ on $(w^{\mathrm{geo}}, w^{\mathrm{fuel}}, w^{\mathrm{reg}}, \mathrm{SameSector})$ within tercile, with Newey-West (lag 4) standard errors on the FM time series. Within-event tercile assignment uses the Herfindahl index of 13F manager shares-of-shares at the most recent quarter on or before the announcement date.

\begin{table}[H]
\centering
\caption{Fuel-similarity coefficient by ($H$, HHI tercile)}
\label{tab:horizon_ownership}
\begin{tabular}{ccccc}
\toprule
$H$ & T1 (dispersed) $\hat\gamma_{\text{fuel}}$ ($t$, $T$) & T3 (concentrated) $\hat\gamma_{\text{fuel}}$ ($t$, $T$) & Difference T3 - T1 (Welch $t$) \\
\midrule
1  & $+0.41$ ($+0.88$, 80) & $-1.62$ ($-0.80$, 81) & $-2.02$ ($-0.97$) \\
3  & $+2.04$ ($+2.79$, 80) & $-7.86$ ($-3.43$, 81) & $-9.90$ ($-4.12$)*** \\
6  & $-1.65$ ($-1.76$, 80) & $-3.92$ ($-0.95$, 81) & $-2.27$ ($-0.53$) \\
12 & $-1.86$ ($-1.09$, 80) & $+1.16$ ($+0.13$, 81) & $+3.03$ ($+0.34$) \\
24 & $-0.01$ ($-0.01$, 80) & $+12.91$ ($+0.96$, 81) & $+12.92$ ($+0.96$) \\
\bottomrule
\end{tabular}
\begin{flushleft}\footnotesize
Notes: Sample restricted to US-listed firms with 13F coverage at the event quarter. T = number of FM events with at least 8 firms within tercile. Welch $t$ for $\hat\gamma^{T3} - \hat\gamma^{T1}$ assumes independence of the two FM time series; *** $p < 0.001$. The T1 sign-reversal at $H = 3$ (consistent with retail-flow misallocation) decays toward zero by $H = 24$. T3 attains its most negative value at $H = 3$ and reverts at long horizons; the rising NW standard error from 2.29 at $H = 3$ to 13.41 at $H = 24$ indicates that long-horizon inference is dominated by noise rather than informative pattern.
\end{flushleft}
\end{table}

The $H = 3$ discrimination (Welch $t = -4.12$) confirms the institutional-pricing mechanism documented in Section~\ref{sec:inst_heterogeneity} at the horizon dimension. Beyond $H = 3$, both terciles' point estimates are statistically indistinguishable from zero, so the long-horizon decay analysis cannot distinguish between systematic-risk and mispricing readings on its own; the headline-window split provides the empirical anchor.
