...
When a coal plant retires, the stock prices of technologically similar utilities fall while those of geographically proximate utilities rise. The same physical event produces opposite-signed spillovers depending on how firms are connected through the spatial network. This finding has broad implications for how financial markets assess transition risk, because the dominant assessment tools operate at the firm level and miss the network dimension entirely.

The scale of the challenge is large. Meeting the 2$^\circ$C target requires leaving over 80 percent of coal reserves unextracted, concentrating stranded asset exposure in specific geographies, with up to \$24 trillion in global financial assets potentially at risk \citep{McGlade2015, Dietz2016}. Markets broadly underestimate this exposure, a difficulty compounded by deep Knightian uncertainty propagating across carbon dynamics, temperature dynamics, and damage functions simultaneously \citep{StroebelWurgler2021, Barnett2020}.

The financial industry's response has been firm-level scoring: ESG ratings, implied temperature scores, and target validation frameworks. This approach suffers from well-documented limitations. Major rating providers disagree on basic assessments of the same company, with pairwise correlations as low as 38 percent \citep{Berg2022}. Mandatory sustainability reporting fails to produce consistent data, because underlying incentives create strategic variation in what companies disclose \citep{Christensen2021}. Corporate climate targets frequently fail basic integrity criteria, and markets exhibit weak reactions when firms miss stated goals \citep{JiangKimLu2025}. Institutional investors overwhelmingly believe climate risks are financially material yet report lacking adequate tools to measure portfolio-level exposure \citep{Krueger2020}.

The problem is not a shortage of data or disclosure mandates, but a conceptual gap: firm-level approaches cannot capture risks that originate in the network connecting firms.
...
Two obstacles have prevented prior work from isolating spatial transmission in transition risk. First, measuring network exposure requires plant-level geolocation data at global scale, which became available only recently through comprehensive registries such as the Global Energy Monitor. Second, separating contagion from competitive effects requires independently constructed network layers whose exclusion restrictions can be tested, a design that standard peer-effect studies using industry classification or geographic proximity alone cannot achieve.
...
Industry-standard assessment tools treat each firm in isolation. ESG ratings, implied temperature scores, and TCFD-aligned disclosures all assume markets can continuously and accurately price firm-level commitments. We propose a different framework: the \textbf{credibility gap} is fundamentally an information friction. Investors observe the physical network of power utilities and each firm's legacy asset exposure, but they cannot reliably assign a directional price to transition commitments until binding policy removes managerial and political discretion.

We frame transition risk as an asset-pricing problem on a multi-layered spatial network under regulatory uncertainty. We derive cross-sectional pricing implications from a conditional valuation framework without solving for general equilibrium; the pricing equation constrains the relationship between spatial exposure and returns without fully specifying investor preferences or market clearing. This framework yields two core short-horizon insights. First, spatial transmission operates through distinct channels with opposing signs. Second, when the regulatory regime is uncertain, idiosyncratic shocks (e.g., plant-level retirements) act as noisy signals that increase return volatility without a clear directional mean. Conversely, binding phase-out laws partially resolve uncertainty, compress volatility, and generate negative returns proportional to firms' own fossil exposure.
...
\end{prediction}
\noindent\textit{Derivation:} With $p_t \in (0,1)$, Equation~\eqref{eq:variance} shows that an increase in $s_j$ strictly increases $\mathrm{Var}_t(V_{i,t})$. To see why the directional effect is ambiguous, let $y$ denote the announcement signal with likelihoods $L_1 = \Pr(y \mid \omega=1)$ and $L_0 = \Pr(y \mid \omega=0)$. Bayes' rule gives $p_t' = p_t L_1 / [p_t L_1 + (1-p_t)L_0]$. If the signal is noisy ($L_1 \approx L_0$), then $p_t' - p_t$ is small. The mean effect in Equation~\eqref{eq:valuation} remains ambiguous because the positive $\gamma_G$ and negative $\gamma_F$ channels offset, while the variance effect is strictly positive (see Appendix~\ref{app:proofs} for the full derivation).
...
The sample comprises global listed power utilities identified by SIC codes 4911, 4931, and 4932 (electric services) and NAICS 2211 in Compustat Global/NA. We cross-reference these firms with plant ownership records in the Global Energy Monitor (GEM) Global Power Plant Tracker (and GeoAsset where available). We require firms to have (i)~at least one operating power plant identifiable in GEM or GeoAsset, (ii)~financial statements in Compustat Global/NA, and (iii)~equity return data in CRSP (US firms) or Datastream (non-US firms). The final sample comprises 389 firms across 50+ countries. Figure~\ref{fig:global_sample} maps the global distribution of sample firms. The event set includes 179 first-mover retirements with exact or announcement dates, 92 EIA-860 retirement announcements, 27 national or subnational phase-out laws (14 Tier-1 binding shocks), and 176 events in the portfolio-sort analysis. Summary statistics appear in Table~\ref{tab:summary_stats}, and Appendix~\ref{app:data} details the regional composition.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig1_global_sample.pdf}
\caption{Global Distribution of Sample Firms}
\label{fig:global_sample}
\begin{flushleft}\footnotesize
Notes: Locations of 389 listed power utilities in the analysis sample, positioned at capacity-weighted plant centroids from GEM/GeoAsset. Circle size proportional to total installed capacity.
\end{flushleft}
\end{figure}
...
The four fundamental parameters $(\lambda,\rho,\kappa,\alpha)$ are constructed from three core databases. Financial statements from Compustat Global/NA provide leverage, an operating-ROA proxy for return spread, and an interest-coverage proxy for cash-flow adequacy. Plant-level asset data from GEM/GeoAsset provide legacy intensity. We use equity returns from CRSP (US) and Datastream (non-US) for return-based specifications and market adjustment. Full variable definitions and supplementary sources appear in Appendix~\ref{app:data}.
...
Figure~\ref{fig:portfolio_sorts} and Table~\ref{tab:portfolio_sorts} provide non-parametric confirmation through portfolio sorts.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_portfolio_sorts.pdf}
\caption{Portfolio Sort Returns by Spatial Exposure Channel}
\label{fig:portfolio_sorts}
\begin{flushleft}\footnotesize
Notes: Mean three-month CARs by quintile of fuel-similarity (Panel A) and geographic-proximity (Panel B) exposure, across 176 first-mover retirement events. Returns are market-adjusted using vwretd. Q5$-$Q1 spreads and $t$-statistics annotated.
\end{flushleft}
\end{figure} The fuel-exposure spread (Q5$-$Q1 $= -5.45\%$, $t=-7.89$) and geographic-exposure spread (Q5$-$Q1 $= +6.03\%$, $t=+9.41$) confirm the opposing-sign structure without imposing linearity. The channel split between the two, $+11.48$ percentage points, is the most precisely estimated result in the paper ($t=14.00$). A long-short portfolio buying geographic Q5 and selling fuel Q5 earns $+8.33\%$ ($t=3.94$), suggesting economic materiality. To interpret these magnitudes: the fuel-similarity Q5 firms experience 5.45 percentage points lower three-month abnormal returns than Q1 firms, equivalent to approximately 22 percent annualised. The long-short portfolio earns 8.33 percent over three months, or roughly 33 percent annualised before transaction costs.
...
The credibility gap is the paper's central finding. We use realised return volatility as a diagnostic for the information friction, not as a claim about risk premia. The opposing channels inject variance without directional content; we observe this as elevated return dispersion around ambiguous events and compressed dispersion around resolving ones. The pricing consequence emerges in the next subsection: when policy credibility rises, the fuel channel strengthens and directional repricing follows.\footnote{Under financial constraints, the volatility increase has direct consequences even with risk-neutral investors: higher cash-flow variance raises the probability that net worth falls below credit-rationing thresholds, reducing firm value through the concavity of the constrained value function \citep{FrootScharfsteinStein1993}. Given the mean leverage of 0.40 in our sample, this channel is plausibly operative, but we do not test it directly.}
...
Table~\ref{tab:ets_interaction} reports the results. The fuel-similarity interaction with ETS presence is strongly negative ($-3.24$, $t=-3.41$, one-sided $p=0.0003$, as the model predicts $\beta_2 < 0$): for firms operating under a carbon pricing regime, the stranding channel is substantially amplified. This directly tests the model's prediction: the coefficient on $w^{\text{fuel}}$ in Equation~\eqref{eq:valuation} is $-p_t \gamma_F$, so a higher $p_t$ (as induced by ETS) yields a more negative spatial effect. Economically, moving from a non-ETS to an ETS jurisdiction amplifies the fuel contagion penalty by 3.24 percentage points per unit of fuel-similarity exposure, roughly tripling the magnitude of the base fuel channel.
...
Portfolio sorts by ETS reinforce the pattern (Figure~\ref{fig:ets_interaction}): the fuel-exposure spread is $-2.54\%$ for ETS firms versus $+1.15\%$ for non-ETS firms (difference $t=-3.66$). Carbon pricing does not create the stranding channel, but it amplifies it by raising the credibility of the transition regime.

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{figures/fig_ets_interaction.pdf}
\caption{Fuel-Similarity Quintile Spread by ETS Status}
\label{fig:ets_interaction}
\begin{flushleft}\footnotesize
Notes: Fuel-similarity Q5$-$Q1 portfolio spread for firms operating under emissions trading systems versus firms without ETS coverage. Difference and $t$-statistic annotated.
\end{flushleft}
\end{figure}
